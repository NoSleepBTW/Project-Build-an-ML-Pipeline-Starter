...
def test_row_count(data: pd.DataFrame) -> None:
    """Test that the dataset has a plausible number of rows.
...
def test_price_range(data: pd.DataFrame, min_price: float, max_price: float) -> None:
    """Test that all prices fall within the configured [min_price, max_price] range.

    Args:
        data: Input DataFrame to test
        min_price: Minimum accepted price (from config, not hard-coded)
        max_price: Maximum accepted price (from config, not hard-coded)
    """
